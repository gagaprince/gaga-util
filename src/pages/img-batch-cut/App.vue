<template>
  <div class="img-corp-container">
    <el-row class="my-row" :gutter="24">
      <el-col :span="6">
        <el-space>
          ...
          <el-input v-model="formObj.width" type="number" placeholder="宽" />
        </el-space>
      </el-col>
      <el-col :span="6">
        <el-space>
          <el-text> height: </el-text>
          <el-input v-model="formObj.height" type="number" placeholder="高" />
        </el-space>
      </el-col>
    </el-row>
    <el-row class="my-row" :gutter="24">
    ...
    <el-row class="my-row" :gutter="24">
      <el-col :span="24">
        ...
        >
          ...
          <template #tip>
            <div class="el-upload__tip">图片文件大小小于100M</div>
          </template>
        </el-upload>
      </el-col>
    </el-row>
    <el-row class="my-row" :gutter="24">
      <el-col :span="24">
        <div v-if="fileList.length > 0" class="preview-img-list">
          <div
            v-for="(fileItem, index) in fileList"
            :key="fileItem.uid"
            class="preview-img-item"
            :style="{
              width: fileItem.fileInfo.showWidth + 'px',
              height: fileItem.fileInfo.showHeight + 'px',
            }"
          >
            ...
            <div class="cp-frame" @click="cpFile(index)">
              <el-icon><CopyDocument /></el-icon>
            </div>
            <div class="del-frame" @click="deleteFile(index)">
    ...
    <el-dialog v-model="showDialog" title="进度" width="500">
...
import VueCropper from "./components/imgCorp/vue-cropper.vue";
import VideoCorp from "./components/videoCorp/video-corp.vue";
  ...
  methods: {
    ...
    packageImg(fileInfo, idx, zip, width, height) {
      ...
      return new Promise((res) => {
        this.$refs[`cropper_${uid}`][0].getCropBlobSelf(
          async (data) => {
    ...
    async download() {
      ...
      for (let i = 0; i < this.fileList.length; i++) {
        ...
        await this.packageImg(
          tmpFile,
          Number(i) + Number(startIndex),
          zip,
          width,
          height
        );
